...
class InputMistakeBookDialog(val id: Int, context: Context, themeResId: Int, private val listener: OnClickListener) :
        Dialog(context, themeResId) {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.input_mistake_book_dialog)
        initView()
    }

    private fun initView() {
        if (id != 0) {
            findViewById<TextView>(R.id.tvDialogTitle).text = context.getString(R.string.update_mistakes_collection)
        }
